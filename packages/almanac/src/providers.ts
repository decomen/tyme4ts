 ...
 */
export function mapOfficialActivities(facts: AdaptedDayFacts): readonly PublicActivityRecord[] {
  const map = (originalName: string, conclusion: ActivityConclusion): PublicActivityRecord => ({
 ...
 */
export function mapModelKnowledgeActivities(facts: AdaptedDayFacts, rulePackage: LoadedRulePackage): readonly AlmanacActivityRecord[] {
  return rulePackage.rules
    .filter((rule) => rule.conditionKey === `duty=${facts.duty}`)
    .map((rule): AlmanacActivityRecord => {
      ...
      const provenance = {
        ...
        reviewStatus: rule.reviewStatus === 'reviewing' ? 'collating' as const : rule.reviewStatus,
      }
 ...
 */
export function createPublicAnnotations(facts: AdaptedDayFacts): readonly AlmanacAnnotation[] {
  const upstream = { source: 'tyme4ts@1.5.2' } as const
  ...
  const annotations: AlmanacAnnotation[] = [
    {
