...
export class AlmanacService {
   ...
   */
  constructor(
    private readonly adapter: Tyme4tsCalendarAdapter,
    private readonly rulePackage?: LoadedRulePackage,
  ) {}
  ...
  getDay(context: CalculationContext): AlmanacDay {
    ...
    if (year === undefined || month === undefined || day === undefined) throw new RangeError('无效民用日期')
    const facts = this.adapter.getDay(year, month, day)
    const activities = mapOfficialActivities(facts).map((item): AlmanacActivityRecord => ({ ...item, ruleHits: [] }))
    ...
    return {
      ...
      hours: facts.hours.map((hour) => ({
        ...
        annotations: [],
        activities: [
          ...hour.recommends.map((name): AlmanacActivityRecord => ({ activityId: `hour.${name}`, canonicalName: name, originalName: name, conclusion: 'recommend', ruleHits: [] })),
          ...hour.avoids.map((name): AlmanacActivityRecord => ({ activityId: `hour.${name}`, canonicalName: name, originalName: name, conclusion: 'avoid', ruleHits: [] })),
        ],
      })),
    }
  }
...
export class SnapshotRepository {
  private readonly snapshots = new Map<string, AlmanacSnapshot>()
  ...
  rollback(snapshotId: string): void {
    if (!this.snapshots.has(snapshotId)) throw new Error(`快照不存在: ${snapshotId}`)
  ...
  verifyRecalculation(snapshotId: string, service: AlmanacService, context: CalculationContext): {
    ...
    if (recorded === undefined) throw new Error(`快照不存在: ${snapshotId}`)
