...
export class RulePackageRegistry {
     ...
     */
    publish(rulePackage) {
        if (this.packages.has(rulePackage.packageVersion))
            throw new Error(`规则包版本已存在: ${rulePackage.packageVersion}`);
    ...
    /** 将当前发布指针回滚到已有历史版本。 */
    rollback(packageVersion) {
        if (!this.packages.has(packageVersion))
            throw new Error(`规则包版本不存在: ${packageVersion}`);
