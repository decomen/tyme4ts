...
function flatten(value, prefix = '') {
    ...
    if (Array.isArray(value)) {
        ...
        for (const [index, child] of value.entries()) {
            for (const [childPath, item] of flatten(child, `${prefix}.${index}`))
                output.set(childPath, item);
        }
    }
        ...
        for (const [key, child] of Object.entries(value).sort(([left], [right]) => left.localeCompare(right))) {
            ...
            for (const [childPath, item] of flatten(child, path))
                output.set(childPath, item);
        }
    }
    else {
 ...
 */
export function compareAdapterResult(actual, expected, allowedPaths) {
    ...
    for (const path of paths) {
        ...
        if (hasActual && hasExpected && Object.is(actualValue, expectedValue))
            continue;
        const kind = !hasExpected ? 'added' : !hasActual ? 'removed' : 'changed';
